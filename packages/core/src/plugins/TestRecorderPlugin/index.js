...
exports.TestRecorderPlugin = exports.isSelectAll = void 0;
...
const copy = (text) => {
    ...
    try {
        ...
        // logInfo(`TestRecorderPlugin: copy${result}`);
    }
    catch (error) {
        // logError(error);
    }
...
function isSelectAll(event) {
    return event.key.toLowerCase() === 'a' && (environment_1.IS_APPLE ? event.metaKey : event.ctrlKey);
}
...
function useTestRecorder(editor) {
    ...
    const getCurrentEditor = (0, react_1.useCallback)(() => editor, [editor]);
...
${steps.map(formatStep).join(`\n`)}
            ...
            if (lastStep) {
                if (lastStep.name === name) {
                    if (name === 'type') {
                        ...
                        return [...steps.slice(0, currentIndex), Object.assign(Object.assign({}, lastStep), { value: lastStep.value + value })];
                    }
                    // for other events we bump the counter if their values are the same
                    if (lastStep.value === value) {
                        return [...steps.slice(0, currentIndex), Object.assign(Object.assign({}, lastStep), { count: lastStep.count + 1 })];
                    }
                }
            }
    ...
    const button = (React.createElement("button", { type: "button", id: "test-recorder-button", className: `editor-dev-button ${isRecording ? 'active' : ''}`, onClick: () => toggleEditorSelection(getCurrentEditor()), title: isRecording ? 'Disable test recorder' : 'Enable test recorder' }));
    const output = isRecording ? (React.createElement("div", { className: "test-recorder-output" },
        React.createElement("div", { className: "test-recorder-toolbar" },
            React.createElement("button", { type: "button", className: "test-recorder-button", id: "test-recorder-button-snapshot", title: "Insert snapshot", onClick: onSnapshotClick }),
            React.createElement("button", { type: "button", className: "test-recorder-button", id: "test-recorder-button-copy", title: "Copy to clipboard", onClick: onCopyClick }),
            React.createElement("button", { type: "button", className: "test-recorder-button", id: "test-recorder-button-download", title: "Download as a file", onClick: onDownloadClick })),
        React.createElement("pre", { id: "test-recorder", ref: preRef }, templatedTest))) : null;
    return [button, output];
}
const TestRecorderPlugin = () => {
